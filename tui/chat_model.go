	...
	case tea.KeyMsg:
        ...
        if m.settingsOpen {
            ...
            case tea.KeyEnter:
                ...
                if m.tmpTopK > 0 { m.topK = m.tmpTopK }
                if m.tmpTemp < 0 { m.tmpTemp = 0 }
                if m.tmpTemp > 1 { m.tmpTemp = 1 }
            ...
            case tea.KeyRunes:
                if len(km.Runes) == 1 {
                    ...
                    case 't', 'T':
                        ...
                        return m, nil
					case '[':
						if m.tmpTopK <= 0 { m.tmpTopK = m.topK }
						if m.tmpTopK > 1 { m.tmpTopK-- }
						return m, nil
					case ']':
						if m.tmpTopK <= 0 { m.tmpTopK = m.topK }
						m.tmpTopK++
            ...
            if km.Type == tea.KeyTab {
                ...
                if m.settingsFocus == 0 { m.modelInput.Focus(); m.hostInput.Blur() } else { m.modelInput.Blur(); m.hostInput.Focus() }
                return m, nil
            }
        ...
        case key.Matches(msg, m.keys.Settings):
            if !m.settingsOpen { m.openSettings() } else { m.settingsOpen = false }
            return m, nil
...
func (m *ChatModel) maybeLLM(question string) tea.Cmd {
    if !m.llmEnabled { return func() tea.Msg { return nil } }
    host := m.ollamaHost
    model := m.modelName
    opts := m.llmOpts
    topK := m.topK
    ...
    return func() tea.Msg {
        idx, err := func() (search.Index, error) {
            if m.indexProvider != nil { return m.indexProvider() }
            return search.OpenOrCreate("")
        }()
        if err != nil { return Message{Role: "assistant", Content: "LLM 前置檢索錯誤: "+err.Error(), TS: time.Now()} }
        defer idx.Close()
        snippets, err := idx.Query(question, topK, nil)
        if err != nil { return Message{Role: "assistant", Content: "LLM 前置檢索錯誤: "+err.Error(), TS: time.Now()} }
        var ctxb strings.Builder
        for i, s := range snippets {
            if i > 0 { ctxb.WriteString("\n---\n") }
            if s.Excerpt != "" { ctxb.WriteString(s.Excerpt) } else { ctxb.WriteString(s.NoteID) }
        }
        ...
        var llm agent.LLM
        if m.llmProvider != nil { llm = m.llmProvider(host, model) } else { llm = agent.NewClient(host, model) }
...
func (m *ChatModel) openSettings() {
    ...
    if m.topK <= 0 { m.topK = 5 }
    m.tmpTopK = m.topK
    m.tmpTemp = m.llmOpts.Temperature
    hi := textarea.New(); hi.SetWidth(80); hi.SetHeight(1); hi.SetValue(m.ollamaHost)
    mi := textarea.New(); mi.SetWidth(40); mi.SetHeight(1); mi.SetValue(m.modelName)
    mi.Focus(); hi.Blur()
    m.hostInput = hi
    m.modelInput = mi
    m.settingsOpen = true
...
func (m *ChatModel) renderSettings() string {
    ...
    if m.settingsErr != "" { lines = append(lines, "Error: "+m.settingsErr) }
    if m.connStatus != "" { lines = append(lines, "Conn: "+m.connStatus) }
    panel := lipgloss.NewStyle().Border(lipgloss.RoundedBorder()).Padding(1, 2).Width(m.width).Render(strings.Join(lines, "\n"))
    return panel
}

func focusMark(active bool) string { if active { return " [*]" } ; return "" }

// connProbe 進行最小連線測試：嘗試 GET 到 host 根路徑。
func connProbe(host string) string {
    if strings.TrimSpace(host) == "" { return "host empty" }
    client := &http.Client{Timeout: 2 * time.Second}
    req, err := http.NewRequest("GET", host, nil)
    if err != nil { return "invalid host" }
